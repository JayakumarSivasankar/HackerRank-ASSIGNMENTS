...
public class AnagramDemo2 {
...

	{
		...
		// System.out.println(tempArray1.length);

		for (int k = 0; k < tempArray1.length; k++) {
			if (String.valueOf(tempArray1).charAt(k) != String.valueOf(tempArray2).charAt(k)) {
				count += 1;
			}
		}
	...
	public void noOfManipulation(String[] s1, String[] s2) {
		...
		for (int i = 0; i < l1.size(); i++) {
			...
			if (l1.get(i).equals(l2.get(i))) {
				lm.put(i, 0);
			} else if (sortChar(l1.get(i)).equals(sortChar(l2.get(i)))) {
				lm.put(i, 0);
